...
public class object_move_you : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        lines = File.ReadAllLines("best(manual).txt");
        ...
        transform.position = new Vector3(startX, 1.084737f, startZ);
    }
    void Update()
    {
        ...
        {
            transform.position = new Vector3(transform.position.x, 1.084737f, transform.position.z);
        }
        if ((Input.GetKey(KeyCode.E) || Input.GetKey(KeyCode.T)) && ((Input.GetKey(KeyCode.LeftShift) && Input.GetKey(KeyCode.RightShift)) == false))
        {
            if (fastup)
            {
                if (fast <= fastwsad*35)
                {
                    if (rlcheck == false)
                    {
                        fast += (((7 - fastwsad) * 0.013f) + 0.1f);
                        if (musicPlayer.isPlaying == false)
                        {
                            musicPlayer.clip = speedkeep;
                            musicPlayer.loop = false;
                            musicPlayer.volume = (fast / 220);
        ...
        else if ((Input.GetKey(KeyCode.D)) && ((Input.GetKey(KeyCode.LeftShift) && Input.GetKey(KeyCode.RightShift)) == false))
        {
            if (fastup)
            {
                if (fast <= fastwsad*35)
            ...
            if (wsad == 0)
            {
                ...
                if (fast > 0.0f)
                {
                    ...
                    else if (Input.GetKey(KeyCode.A))
                    {
                        if (plane)
                        {
                            transform.eulerAngles = new Vector3(transform.eulerAngles.x, transform.eulerAngles.y - 1.2f, transform.eulerAngles.z);
                        }
                    }
                    else if (Input.GetKey(KeyCode.G))
                    {
    ...
    void OnCollisionEnter(Collision other)
    {
        ...
        {
            Vector3 target = new Vector3(transform.position.x - (fast / 11), transform.position.y, transform.position.z);
        ...
        {
            Vector3 target = new Vector3(transform.position.x, transform.position.y, transform.position.z + (fast / 11));
            Vector3 velo = Vector3.zero;
            transform.position = Vector3.SmoothDamp(transform.position, target, ref velo, 0.1f);
            ...
            if (other.gameObject.tag == "track")
            {
                ...
                if (eulardata == 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            transform.position = new Vector3(transform.position.x - (fast*0.8f), transform.position.y, transform.position.z);
                        }
                ...
                if (eulardata == 90)
                {
                    ...
                    else
                    {
                        ...
                        {
                            transform.position = new Vector3(transform.position.x, transform.position.y, transform.position.z - (fast*0.8f));
                        }
        ...
        if (other.gameObject.tag == "kill")
        {
            ...
            if (checkpoint == 1)
            {
                transform.position = new Vector3(GameObject.Find("checkpoint1"+mapname).transform.position.x, 1.084737f, GameObject.Find("checkpoint1" + mapname).transform.position.z);
                transform.eulerAngles = new Vector3(0, GameObject.Find("checkpoint1"+mapname).transform.eulerAngles.y+180, 0);
            }
            if (checkpoint == 2)
            {
                transform.position = new Vector3(GameObject.Find("checkpoint2"+mapname).transform.position.x, 1.084737f, GameObject.Find("checkpoint2"+mapname).transform.position.z);
                transform.eulerAngles = new Vector3(0, GameObject.Find("checkpoint2"+mapname).transform.eulerAngles.y + 180, 0);
            }
            if (checkpoint == 3)
            {
                transform.position = new Vector3(GameObject.Find("checkpoint3"+mapname).transform.position.x, 1.084737f, GameObject.Find("checkpoint3"+mapname).transform.position.z);
                transform.eulerAngles = new Vector3(0, GameObject.Find("checkpoint3"+mapname).transform.eulerAngles.y + 180, 0);
            }
            if (checkpoint == 4)
            {
                transform.position = new Vector3(GameObject.Find("checkpoint4"+mapname).transform.position.x, 1.084737f, GameObject.Find("checkpoint4"+mapname).transform.position.z);
                transform.eulerAngles = new Vector3(0, GameObject.Find("checkpoint4"+mapname).transform.eulerAngles.y + 180, 0);
            }
            if (checkpoint == 5)
            {
                transform.position = new Vector3(GameObject.Find("checkpoint5"+mapname).transform.position.x, 1.084737f, GameObject.Find("checkpoint5" + mapname).transform.position.z);
                transform.eulerAngles = new Vector3(0, GameObject.Find("checkpoint5" + mapname).transform.eulerAngles.y + 180, 0);
            }
            if (checkpoint == 6)
            {
                transform.position = new Vector3(GameObject.Find("checkpoint6" + mapname).transform.position.x, 1.084737f, GameObject.Find("checkpoint6" + mapname).transform.position.z);
                transform.eulerAngles = new Vector3(0, GameObject.Find("checkpoint6" + mapname).transform.eulerAngles.y + 180, 0);
            }
            if (checkpoint == 7)
            {
                transform.position = new Vector3(GameObject.Find("checkpoint7" + mapname).transform.position.x, 1.084737f, GameObject.Find("checkpoint7" + mapname).transform.position.z);
                transform.eulerAngles = new Vector3(0, GameObject.Find("checkpoint7" + mapname).transform.eulerAngles.y + 180, 0);
            }
            if (checkpoint == 8)
            {
                transform.position = new Vector3(GameObject.Find("checkpoint8" + mapname).transform.position.x, 1.084737f, GameObject.Find("checkpoint8" + mapname).transform.position.z);
                transform.eulerAngles = new Vector3(0, GameObject.Find("checkpoint8" + mapname).transform.eulerAngles.y + 180, 0);
            }
            if (checkpoint == 9)
            {
                transform.position = new Vector3(GameObject.Find("checkpoint9" + mapname).transform.position.x, 1.084737f, GameObject.Find("checkpoint9" + mapname).transform.position.z);
                transform.eulerAngles = new Vector3(0, GameObject.Find("checkpoint9" + mapname).transform.eulerAngles.y + 180, 0);
            }
            fast = 0.0f;
        }
        if (other.gameObject.tag == "finish")
        {
            ...
            else if (finaltime == true)
            {
                ...
                if (finishfirstcheck)
                {
                    ...
                    besttime.text = M.ToString();
                    besttimecheck = M;
                    StreamWriter sw = new StreamWriter("best(manual).txt");
                    if(mapselect.mapselectcheck == 0)
